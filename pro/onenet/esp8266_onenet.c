...
baud_ready:
  ...
  joined = ESP8266_WaitForEither("WIFI GOT IP", "OK", 45000U);
  if (joined == 0U)
  {
    (void)snprintf(cmd, sizeof(cmd), "AT+CWJAP?");
    ESP8266_DrainRx();
    if (HAL_UART_Transmit(esp_uart, (uint8_t *)cmd, (uint16_t)strlen(cmd), 1000U) == HAL_OK)
    {
      (void)HAL_UART_Transmit(esp_uart, (uint8_t *)"\r\n", 2U, 1000U);
      joined = ESP8266_WaitFor(ONENET_WIFI_SSID, 3000U);
    }
  }

  if (joined == 0U)
  {
    joined = ESP8266_SendCommand("AT+CIFSR", "STAIP", 3000U);
  }

  if (joined == 0U)
  {
    if (strstr(esp_rx_buffer, "+CWJAP:1") != NULL)
    {
      esp_last_error = ESP8266_ONENET_ERROR_JOIN_AP_TIMEOUT;
    }
    else if (strstr(esp_rx_buffer, "+CWJAP:2") != NULL)
    {
      esp_last_error = ESP8266_ONENET_ERROR_JOIN_AP_PASSWORD;
    }
    else if (strstr(esp_rx_buffer, "+CWJAP:3") != NULL)
    {
      esp_last_error = ESP8266_ONENET_ERROR_JOIN_AP_NOT_FOUND;
    }
    else if (strstr(esp_rx_buffer, "+CWJAP:4") != NULL)
    {
      esp_last_error = ESP8266_ONENET_ERROR_JOIN_AP_FAILED;
    }
    else if (strstr(esp_rx_buffer, "FAIL") != NULL)
    {
      esp_last_error = ESP8266_ONENET_ERROR_JOIN_AP_FAIL_RESPONSE;
    }
    else
    {
      esp_last_error = ESP8266_ONENET_ERROR_JOIN_AP;
...
static uint8_t ESP8266_ConnectMQTT(void)
{
  ...
  if (strlen(ONENET_MQTT_TOKEN) > 64U)
  {
  ...
  if (i >= (sizeof(mqtt_hosts) / sizeof(mqtt_hosts[0])))
  {
    ...
    {
      esp_last_error = ESP8266_ONENET_ERROR_MQTT_CONNECT_DISCONNECTED;
    }
    else if ((strstr(esp_rx_buffer, "ERROR") != NULL) ||
             (strstr(esp_rx_buffer, "FAIL") != NULL))
    {
      esp_last_error = ESP8266_ONENET_ERROR_MQTT_CONNECT_FAIL_RESPONSE;
    }
    else
    {
...
uint8_t ESP8266_OneNET_PostTempHumi(float temperature, float humidity, float lux, int heartrate, float spo2)
{
  ...
  (void)snprintf(payload, sizeof(payload),
                 "{\"id\":\"123\",\"version\":\"1.0\",\"params\":{\"TEMP\":{\"value\":%.1f},\"HUMI\":{\"value\":%.1f},\"LUX\":{\"value\":%.1f},\"HR\":{\"value\":%d},\"SPO2\":{\"value\":%.1f}}}",
                 temperature,humidity,lux,heartrate,spo2);
  (void)snprintf(cmd, sizeof(cmd),
