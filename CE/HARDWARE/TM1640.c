...
void send_LED_Display(unsigned char address,uint16 data,unsigned char type); 
void LED_DisplayClear(void); 
void TM1640_Init(void)
{
    P1SEL &= ~0x30;     //设置P1.4、P1.5为普通IO口  
    P1DIR |= 0x30;      //P1.4、P1.5定义为输出
    init_tm1640();  
    LED_DisplayClear();//清楚显示
}
...
void LED_DisplayClear(void)  
{
  unsigned char SEG_1 = 10,SEG_2 = 10,SEG_3 = 10,SEG_4 = 10;
...
void send_z(uint8 address,uint8 date)
{
    unsigned char i;
    start();
    write(address);	//设置起始地址
...
void send(uint8 address, uint8 date)
