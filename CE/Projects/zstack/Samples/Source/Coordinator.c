...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pckt );//接收到消息回调函数
/*********************************************************************
 * @fn      SampleApp_Init
 *
...
{ 
	...
	{
		case TEMP_HUMI_CLUSTERID:  // 接收组合的温湿度数据
			// 新数据格式：sendbuf[0]=temperature, sendbuf[1]=humidity
			// 我们需要组合显示：温度在前两位，湿度在后两位
			uint8 temp = pkt->cmd.Data[0];     // 温度值
			uint8 humi = pkt->cmd.Data[1];     // 湿度值
			...
			// OLED显示：温度和湿度分别显示
			ShowCombinedBuf[10] = temp/10 + 0x30;      // 温度十位
			ShowCombinedBuf[11] = temp%10 + 0x30;      // 温度个位
